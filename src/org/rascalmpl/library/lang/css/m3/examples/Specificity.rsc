...
public void metrics1() {
	...
	list[loc] dirs = [
	 ...
	 //   |home:///Documents/workspace/Rascal/rascal/testCSS/sample-set/web-new/youtube.com.css|
	 	...
	 	|home:///Documents/workspace/Rascal/rascal/testCSS/examples/test.css|
	];
	
	for (d <- dirs) {
		stylesheetAST = createAstFromFile(d);
		
		iprintln(selectors(createM3FromFile(d)));

		//iprintln("<d.file>");
		//iprintln(specificity());
		
		list[str] a = specificity(stylesheetAST);
		
		iprintln(a);
		
		//println(toReal(toReal(sum(a))/toReal(size(a))));
		
		//writeFile(|home:///Documents/workspace/Rascal/rascal/testCSS/specificity/<d.file>|, specificity(stylesheetAST));
	}
}
