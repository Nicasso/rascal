...
public abstract class JavaToRascalConverter extends ASTVisitor {
	...
	JavaToRascalConverter(final TypeStore typeStore, Map<String, ISourceLocation> cache, boolean collectBindings) {
		...
		JavaToRascalConverter.DATATYPE_RASCAL_MESSAGE_DATA_TYPE = typeStore
				.lookupAbstractDataType(DATATYPE_RASCAL_MESSAGE);
		JavaToRascalConverter.DATATYPE_RASCAL_MESSAGE_ERROR_NODE_TYPE = typeStore
		...
		try {
			...
			if (nodeLength > 0) {
				...
				if (end < start && ((node.getFlags() & 9) > 0)) {
					insert(messages,
							values.constructor(DATATYPE_RASCAL_MESSAGE_ERROR_NODE_TYPE,
				...
				return values.sourceLocation(loc, start, nodeLength, compilUnit.getLineNumber(start),
						compilUnit.getLineNumber(end),
						// TODO: only adding 1 at the end seems to work, need to
						// test.
			...
			insert(messages, values.constructor(DATATYPE_RASCAL_MESSAGE_ERROR_NODE_TYPE,
					values.string("Most probably missing dependency"), values.sourceLocation(loc, 0, 0)));
		}
	...
	protected IValueList parseModifiers(int modifiers) {
		...
		for (String constructor : java.lang.reflect.Modifier.toString(modifiers).split(" ")) {
			Set<org.rascalmpl.value.type.Type> exConstr = typeStore
					.lookupConstructor(DATATYPE_RASCAL_AST_MODIFIER_NODE_TYPE, constructor);
