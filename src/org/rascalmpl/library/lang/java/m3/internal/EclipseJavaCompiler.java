...
public class EclipseJavaCompiler {
	...
	public IValue createM3sFromFiles(ISet files, IBool errorRecovery, IList sourcePath, IList classPath,
			IString javaVersion, IEvaluatorContext eval) {
		try {
			TypeStore store = new TypeStore();
			store.extendStore(eval.getHeap().getModule("lang::java::m3::Core").getStore());
			store.extendStore(eval.getHeap().getModule("lang::java::m3::AST").getStore());
			...
			buildCompilationUnits(files, true, errorRecovery.getValue(), sourcePath, classPath, javaVersion,
					(loc, cu) -> {
	...
	public IValue createM3sAndAstsFromFiles(ISet files, IBool errorRecovery, IList sourcePath, IList classPath,
			IString javaVersion, IEvaluatorContext eval) {
		try {
			TypeStore store = new TypeStore();
			store.extendStore(eval.getHeap().getModule("lang::java::m3::Core").getStore());
	...
	public IValue createM3FromString(ISourceLocation loc, IString contents, IBool errorRecovery, IList sourcePath,
			IList classPath, IString javaVersion, IEvaluatorContext eval) {
		try {
			CompilationUnit cu = getCompilationUnit(loc.getPath(), contents.getValue().toCharArray(), true,
					errorRecovery.getValue(), javaVersion, translatePaths(sourcePath), translatePaths(classPath));
	...
	public IValue createAstsFromFiles(ISet files, IBool collectBindings, IBool errorRecovery, IList sourcePath,
			IList classPath, IString javaVersion, IEvaluatorContext eval) {
		try {
			TypeStore store = new TypeStore();
			store.extendStore(eval.getHeap().getModule("lang::java::m3::AST").getStore());
			...
			buildCompilationUnits(files, collectBindings.getValue(), errorRecovery.getValue(), sourcePath, classPath,
					javaVersion, (loc, cu) -> {
						checkInterrupted(eval);
	...
	public IValue createAstFromString(ISourceLocation loc, IString contents, IBool collectBindings, IBool errorRecovery,
			IList sourcePath, IList classPath, IString javaVersion, IEvaluatorContext eval) {
		try {
			CompilationUnit cu = getCompilationUnit(loc.getPath(), contents.getValue().toCharArray(),
					collectBindings.getValue(), errorRecovery.getValue(), javaVersion, translatePaths(sourcePath),
					translatePaths(classPath));
	...
	protected IValue convertToM3(TypeStore store, Map<String, ISourceLocation> cache, ISourceLocation loc,
		...
		for (Object cm : cu.getCommentList()) {
			...
			// Issue 720: changed condition to only visit comments without a
			// parent (includes line, block and misplaced javadoc comments).
	...
	protected void buildCompilationUnits(ISet files, boolean resolveBindings, boolean errorRecovery, IList sourcePath,
			IList classPath, IString javaVersion, BiConsumer<ISourceLocation, CompilationUnit> buildNotifier)
			throws IOException {
		...
		for (IValue p : paths) {
			...
			if (!loc.getScheme().equals("file")) {
				throw RascalRuntimeException
						.io(VF.string("all path entries must have (or resolve to) the file:/// scheme: " + loc), null);
			}
	...
	protected CompilationUnit getCompilationUnit(String unitName, char[] contents, boolean resolveBindings,
			boolean errorRecovery, IString javaVersion, String[] sourcePath, String[] classPath) throws IOException {
	...
	protected ASTParser constructASTParser(boolean resolveBindings, boolean errorRecovery, IString javaVersion,
